...
class AttentiveTransformer(torch.nn.Module):
    def __init__(
        ...
        self.conv = nn.Sequential(
            nn.Conv1d(1, 8, 2),  # torch.size([batch_size, 8, 6])
            nn.LeakyReLU(),
            nn.Conv1d(8, 16, 2),  # torch.size([batch_size, 8, 6])
            nn.LeakyReLU(),
            nn.MaxPool1d(2),  # torch.Size([batch_size, 8, 3])
            nn.ConvTranspose1d(16, 1, 3),
            # nn.Conv1d(16, 1, 2),
            nn.LeakyReLU(),
            # nn.MaxPool1d(2),  # 输出大小：torch.Size([128, 32, 1])
            nn.Flatten(),  # 输出大小：torch.Size([128, 32])
        )
        
        self.linear = nn.Sequential(
            nn.Linear(in_features=5, out_features=output_dim, bias=True),
            nn.Sigmoid(),
        )
    ...
    def forward(self, priors, processed_feat):
        ...
        processed_feat = processed_feat.unsqueeze(dim=1)
        x = self.conv(processed_feat)
        x = self.linear(x)
        x = torch.mul(x, priors)
        return x
